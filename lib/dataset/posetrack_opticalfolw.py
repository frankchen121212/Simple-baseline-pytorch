...
from dataset.Joints_flow_Dataset import JointsDataset
...
class PoseTrackDataset(JointsDataset):
    ...
    def _load_coco_person_detection_results(self):
        ...
        for n_img in range(0, len(all_boxes)):
            ...
            # if det_res['image_id'] == 10003420000:
            #     print('center:{}'.format(center))
            #     print('scale:{}'.format(scale))
            #     file = cv2.imread(img_name, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION)
            #     file_name = '{}_{}bbox.jpg'.format(det_res['image_id'],num_boxes)
            #
            #     x, y, w, h = int(box[0]), int(box[1]), int(box[2]), int(box[3])
            #
            #     cv2.rectangle(file, (x, y),
            #                   (x+w, y+h),
            #                   (0, 0, 255), 2)
            #     cv2.imwrite(os.path.join(self.root, file_name), file)
            joints_3d = np.zeros((self.num_joints, 3), dtype=np.float)
            ...
            kpt_db.append({
                ...
                'box_sc': box_sc,  # [x,y,w,h,score] for nms
                'center': center,
                'scale': scale,
                'score': score,
                'joints_3d': joints_3d,
    ...
    def image_path_from_index(self, index):
        ...
        #suffix = '_mpii_train' if 'train' in self.image_set else '_mpii_test'
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, image_path, track_IDs):
        ...
        if 'test' or 'val' not in self.image_set:
            return {'Null': 0}, 0
    ...
    def _coco_keypoint_results_one_category_kernel(self, data_pack):
        ...
        for img_kpts in keypoints:
            ...
            result = [{'image_id': int(img_kpts[k]['image_id']),
                       'track_id': int(img_kpts[k]['track_id']),
                       'category_id': cat_id,
